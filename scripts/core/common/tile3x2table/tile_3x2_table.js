...
tile_3x2_table.size = '3x2';
tile_3x2_table.type = 'table';
tile_3x2_table.needData = true;

tile_3x2_table_save_initial = function() {
    return defaultSaveData();
};

tile_3x2_table.tile_3x2_table_create = function(ele) {
    var result = tile_3x2_table_save_initial();
    ...
    var newtile = { //the new tile
        ...
        size: tile_3x2_table.size.split('x'),
        type: tile_3x2_table.type
    };
...
tile_3x2_table.tile_3x2_table_save = false;

tile_3x2_table_format_data = function(data, settings, sources) {
    var ret = [];
...
tile_3x2_table.tile_3x2_table_draw = function(ele) {
    ...
    var objects = tile_3x2_table_format_data(ele.parent.data, ele.settings, ele.parent.sources);
    if(objects.length > 0) {
    ...
    tile_3x2_table_draw_back(ele);
};

tile_3x2_table_draw_back = function(ele) { //can be used to update any stuff on back of tile, atm only does sources
    defaultsaveBack(ele);
};