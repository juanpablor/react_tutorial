...
tile_4x4_table.size = '4x4';
tile_4x4_table.type = 'table';
tile_4x4_table.needData = true;

tile_4x4_table_save_initial = function() {
    return defaultSaveData();
};

tile_4x4_table.tile_4x4_table_create = function(ele) {
    var result = tile_4x4_table_save_initial();
    ...
    var newtile = { //the new tile
        ...
        size: tile_4x4_table.size.split('x'),
        type: tile_4x4_table.type
    };
...
tile_4x4_table.tile_4x4_table_save = false;

tile_4x4_table_format_data = function(data, settings, sources) {
    var ret = [];
...
tile_4x4_table.tile_4x4_table_draw = function(ele) {
    ...
    var objects = tile_4x4_table_format_data(ele.parent.data, ele.settings, ele.parent.sources);
    if(objects.length > 0) {
    ...
    tile_4x4_table_draw_back(ele);
};

tile_4x4_table_draw_back = function(ele) { //can be used to update any stuff on back of tile, atm only does sources
    defaultsaveBack(ele);
};